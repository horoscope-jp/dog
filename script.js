...
// ── 犬画像 134枚 ──
const BASE_URL = 'https://joyu-dot-com.xyz/wp-content/uploads/2026/06/';
const DOG_IMAGES = Array.from({ length: 134 }, (_, i) => {
  const n = String(i + 1).padStart(3, '0');
  return `${BASE_URL}cute-dog-${n}.jpg`;
});

const TOP_PAGE_IMAGES = DOG_IMAGES.slice();

const PAGE_DOG_IMG = {
  'page-overall': 'dog-img-overall',
  'page-love':    'dog-img-love',
  'page-work':    'dog-img-work',
  'page-money':   'dog-img-money',
};
...
function applySessionImage(pageId) {
  const imgId = PAGE_DOG_IMG[pageId];
...
document.getElementById('money-next').addEventListener('click', () => { ensureBones(); showPage('page-top'); });
...
function initBones() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const left = i < half
      ? Math.random() * 45
      : 55 + Math.random() * 45;
    bone.style.left = left + '%';
    bone.style.animationDuration = (15 + Math.random() * 7) + 's';
    bone.style.animationDelay = (-Math.random() * 18) + 's';
...
function ensureBones() {
  if (window.innerWidth < 768) return;
  if (document.querySelectorAll('.bone-float').length === 0) {
    initBones();
  }
}
